...
// import Logo from '../../../Images/CCG.png'
...
//               <h1 className="text-xl lg:text-2xl font-bold text-[#d62332]">CAREER GUIDE GLOBAL</h1>
//               <p className="text-xs font-medium text-[#373075] -mt-1">Connecting Education</p>
//             </div>
//           </div>
...
//                 <Link
//                   href={item.href}
//                   className={`flex items-center space-x-1 py-2 font-medium transition-colors duration-200 ${
//                     isActive(item.href) 
//                       ? 'text-[#4fbe99]' 
//                       : 'text-gray-700 hover:text-[#4fbe99]'
//                   }`}
...
//                     <span>{item.name}</span>
//                     {/* Animated underline */}
//                     <div className={`absolute -bottom-1 left-0 h-0.5 bg-[#4fbe99] transition-all duration-300 ${
//                       isActive(item.href) 
//                         ? 'w-full' 
...
//                 <Link
//                   href={item.href}
//                   className={`flex items-center justify-between py-3 transition-colors duration-200 font-medium ${
//                     isActive(item.href)
//                       ? 'text-[#4fbe99]'
//                       : 'text-gray-700 hover:text-[#4fbe99]'
//                   }`}
...
//                         key={dropdownItem.name}
//                         href={dropdownItem.href}
//                         className={`flex items-center space-x-3 py-2 transition-colors duration-200 ${
//                           isActive(dropdownItem.href)
//                             ? 'text-[#4fbe99]'
//                             : 'text-gray-600 hover:text-[#4fbe99]'
//                         }`}
//                       >
//                         <div className="text-[#4fbe99]">{dropdownItem.icon}</div>
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 lg:h-20">
              ...
              <div key={item.name} className="relative group">
                <Link
                  ...
                  className={`flex items-center space-x-1 py-2 font-medium transition-colors duration-200 ${
                    isActive(item.href) 
                      ? 'text-[#d62332]' 
                      : 'text-gray-700 hover:text-[#d62332]'
                  }`}
                  onMouseEnter={() => item.dropdown && setActiveDropdown(item.name)}
                  onMouseLeave={() => !item.dropdown && setActiveDropdown(null)}
                >
                  ...
                  >
                    {item.dropdown.map((dropdownItem) => (
                      <Link
                        key={dropdownItem.name}
                        href={dropdownItem.href}
                        className="flex items-center space-x-3 px-4 py-3 text-gray-700 hover:bg-[#d62332]/10 hover:text-[#d62332] transition-colors duration-200"
                      >
                        <div className="text-[#d62332]">{dropdownItem.icon}</div>
                        <span className="font-medium">{dropdownItem.name}</span>
                      </Link>
                    ))}
                  </div>
          ...
          {/* CTA Buttons */}
          <div className="hidden lg:flex items-center space-x-4">
            <button className="flex items-center space-x-2 px-4 py-2 text-[#d62332] border border-[#d62332] rounded-lg hover:bg-[#d62332]/10 transition-colors duration-200 font-medium">
              <Phone className="w-4 h-4" />
              <span>Call Us</span>
            </button>
            <button className="flex items-center space-x-2 px-6 py-2 bg-[#d62332] text-white rounded-lg hover:bg-[#b51d2a] transition-colors duration-200 font-medium shadow-lg hover:shadow-xl">
              <Calendar className="w-4 h-4" />
              <span>Book Session</span>
              <ArrowRight className="w-4 h-4" />
            </button>
        ...
        <div className="bg-white border-t border-gray-100 shadow-lg">
          <div className="px-4 py-6 space-y-4">
                    ...
                    {item.dropdown.map((dropdownItem) => (
                      <Link
                        ...
                        className={`flex items-center space-x-3 py-2 transition-colors duration-200 ${
                          isActive(dropdownItem.href)
                            ? 'text-[#d62332]'
                            : 'text-gray-600 hover:text-[#d62332]'
                        }`}
                      >
                        <div className="text-[#d62332]">{dropdownItem.icon}</div>
                        <span>{dropdownItem.name}</span>
            ...
            <div className="pt-4 space-y-3 border-t border-gray-100">
              ...
              <button className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-[#d62332] text-white rounded-lg hover:bg-[#b51d2a] transition-colors duration-200 font-medium shadow-lg">
                <Calendar className="w-4 h-4" />
                <span>Book Session</span>
                <ArrowRight className="w-4 h-4" />
              </button>
